...
namespace AgricoveBilling
{
    public partial class AgricoveBilling : Form
    {
        ...
        //This disbles all rows in invoice table and changes the bgcolor of cells to imply which fields are disabled
        private void disable_rows()
        {
            ...
            descBox1.Enabled = true;                                //This makes sure the first descbox stays enabled
        ...
        {
            recursive_up ( i + 1 );                //reach the very end
            try
            {
                if ( descBox [ i + 1 ].Text.Length > 0 )                 //try because what if i+1 doesn't exist?
                {
                    ...
                    //change i color
                    bgColors [ i + 1 , 0 ] = SystemColors.ControlLightLight;                //bgcolro must start one count ahead because it includes header row
                    ...
                    //disable i+1 except descbox
                    descBox [ i + 1 ].Enabled = true;
                    qtyBox [ i + 1 ].Enabled = false;
                    unitBox [ i + 1 ].Enabled = false;
                    ...
                    bgColors [ i + 2 , 3 ] = SystemColors.ButtonFace;

                    //disable and clear i+2 descbox and change its color 
                    try
                    {
                        descBox [ i + 2 ].Clear ();             //try because what if i+2 doesn't exist?
                    ...
                    invoice_table.Refresh ();                    //apply the changed color scheme
                }
            }
            catch
            {
                return;             //has reached the very end
            }
        }
        ...
        {
            if ( !recursive ) // run code only if reciursive flag false (to prevent overwrite of manually entered data loaded recursively from lower rows
            {
                int i = Int32.Parse ( ( sender as TextBox ).Tag.ToString () );            //read its array index
                if ( descBox [ i ].Text.Length > 0 )
                {
                    if ( fetch_price ( descBox [ i ].Text ) != null && fetch_itemtype ( descBox [ i ].Text ) != null )            //check if there is any existing entry
                    {
                    ...
                    try
                    {
                        descBox [ i + 1 ].Enabled = true;                              //enable next row if exists
                        bgColors [ i + 2 , 0 ] = SystemColors.ControlLightLight;        //bgcolors index starts one value ahead because it includes the header of table
                    }
                    catch { }
                }
                else
                {
                    ...
                    unitBox [ i ].SelectedIndex = 0;
                    unitBox [ i ].Enabled = false;
                    bgColors [ i + 1 , 2 ] = SystemColors.ButtonFace;
                    uBox [ i ].Value = 0;
                    uBox [ i ].Enabled = false;
                    bgColors [ i + 1 , 3 ] = SystemColors.ButtonFace;
                    ...
                    try
                    {
                        ...
                        {
                            descBox [ i + 1 ].Enabled = false;                     //else just disable the descbox of next row
                            bgColors [ i + 2 , 0 ] = SystemColors.ButtonFace;
                        }
                    }
                    catch { }
                }
                invoice_table.Refresh ();                        //apply the color change
            }
        }
            ...
            if ( senderGrid.Columns [ e.ColumnIndex ] is DataGridViewButtonColumn && e.RowIndex >= 0 )
            {
                ...
                if ( datasrc.SelectedIndex == 0 )                     //load invoice from search result
                {
                    using ( var dataContext = new DBConnection () )
                    {
                        ...
                        edit_check = 1;                             //this must be called before load_data
                        load_data ( inv );
                        print.Enabled = true;
                        find_Click ( sender , e );
                    }
